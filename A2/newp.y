...
int lines = 1; // Initializing line number

// declare any other variables or functions needed here

%}

%token IF ELSE FOR WHILE DO BREAK CONTINUE RETURN INT FLOAT CHAR VOID DOUBLE SWITCH CASE DEFAULT PRINTLN INCOP DECOP ADDOP MULOP RELOP ASSIGNOP LOGICOP NOT LPAREN RPAREN LCURL RCURL LTHIRD RTHIRD SEMICOLON COMMA ID CONST_INT CONST_FLOAT
...
func_definition : type_specifier ID LPAREN parameter_list RPAREN compound_statement
	{
		...
		$$ = new symbol_info($1->getname() + " " + $2->getname() + "(" + $4->getname() + ") " + $6->getname(), "func_definition");
	}
	| type_specifier ID LPAREN RPAREN compound_statement
	{
		outlog << "At line no: " << lines << " func_definition : type_specifier ID LPAREN RPAREN compound_statement" << endl << endl;
		outlog << $1->getname() << " " << $2->getname() << "() " << $5->getname() << endl << endl;
		$$ = new symbol_info($1->getname() + " " + $2->getname() + "() " + $5->getname(), "func_definition");
	}
	;
...
void yyerror(const char *s) {
	cerr << "Error: " << s << " at line " << lines << endl;
}
...
int main(int argc, char *argv[])
{
    ...
    outlog << "Total lines: " << lines - 1 << endl << endl; // Print number of lines
